(() => {
  ...
  const STAGED_KEY = `heuro_transportes_provisorios_v77_${session.user_id}`;
  ...
  function tableRow(request, mode) {
    ...
    return `<tr data-row-id="${esc(request.id)}" class="${provisional ? 'provisional-row' : ''}">
      ...
      <td><div class="table-actions"><button class="details compact" data-details="${esc(request.id)}" type="button">Detalhes</button>${lifecycleButton(request, mode)}</div></td>
    </tr>`;
  }
  ...
  function renderPending(term = '') {
    const chosen = requests.filter((request) => staged.get(String(request.id)) === 'accept' && matches(request, term)).sort((a, b) => priority(a) - priority(b) || stamp(a) - stamp(b));
    ...
    list.innerHTML = `${tableSection(chosen, 'provisional-accept', 'Aceites selecionados')}${tableSection(waiting, 'pending')}` || '<div class="empty">Nenhum transporte pendente.</div>';
    bindActions();
  }
  ...
  $('commandBack').addEventListener('click', (event) => {
    ...
    leaveScreen(() => location.assign(`./comando.html?v=20260807.77&fresh=${Date.now()}`));
  });
